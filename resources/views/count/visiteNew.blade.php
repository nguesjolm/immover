...
<div class="container mt-5 mb-md-4 py-5">
        ...
        <div class="card card-body p-4 p-md-5 shadow-sm">
          ...
          <div class="mt-md-n3 mb-4"><a class="btn btn-outline-primary btn-lg rounded-pill w-100 d-md-none" href="#account-nav" data-bs-toggle="collapse"><i class="fi-align-justify me-2"></i>Menu</a>
            <div class="collapse d-md-block" id="account-nav">
              <ul class="nav nav-pills flex-column flex-md-row pt-3 pt-md-0 pb-md-4 border-bottom-md">
                ...
                <li class="nav-item mb-md-0 me-md-2 pe-md-1"><a class="nav-link active" href="visitesNew" aria-current="page"><i class="fi-star mt-n1 me-2 fs-base"></i> Visite Nouvelle ({{count(AllVisiteClient($_SESSION['clientID'],0))}})</a></li>
          ...
          <div class="alert alert-primary" role="alert">
             <i class="fi-star mt-n1 me-2 fs-base"></i> Visites Nouvelles({{count(AllVisiteClient($_SESSION['clientID'],0))}})
           </div>
        
        @if(count(AllVisiteClient($_SESSION['clientID'],0))!=0)
          <!-- Favorites grid-->
          <div class="row row-cols-lg-3 row-cols-sm-2 row-cols-1 gy-4 gx-3 gx-lg-4">
            <?php foreach (AllVisiteClient($_SESSION['clientID'],0) as $visite) {?>
            <!-- Item-->
            <div class="col pt-2">
              <div class="position-relative">
                <div class="position-relative mb-3">
                  <button class="btn btn-icon btn-primary btn-xs text-white rounded-circle position-absolute top-0 end-0 m-3 zindex-5" type="button" data-bs-toggle="tooltip" data-bs-placement="left" title="Remove from Favorites"><i class="fi-star"></i></button><img class="rounded-3" src="{{ReadBiensID($visite->idbiens)->img1}}" alt="Article img">
                </div>
                <span class="text-danger"><b>{{ReadOperaID(ReadBiensID($visite->idbiens)->typesoperations_idtypesoperations)->operation}}</b></span>
                <h3 class="mb-2 fs-lg">
                  <a class="nav-link" href="single?biens={{$visite->idbiens}}">{{ReadBiensID($visite->idbiens)->libele}}</a>
                </h3>
                <span>Code visite : {{$visite->codevisites}} </span><br>
                <span><b>Code du biens: {{ReadBiensID($visite->idbiens)->codebiens}}</b></span>
                ...
                <!-- Action :: Effectuer - Solder -->
                <!--<a href="visiteSold?id={{$visite->idbiensclient}}&idbiens={{$visite->idbiens}}&url=visitesNew"><span class="badge rounded-pill bg-success">Solder</span></a>-->
                <a href="visiteEffect?id={{$visite->idbiensclient}}&idbiens={{$visite->idbiens}}&url=visitesNew"><span class="badge rounded-pill bg-warning">Effectuer</span></a>
                <a href="visiteAnnul?id={{$visite->idbiensclient}}&idbiens={{$visite->idbiens}}&url=visitesNew"><span class="badge rounded-pill bg-danger">Annuler</span></a>
        ...
        @else
          <span>Aucune nouvelles visites</span>
        @endif
        </div>
      </div>