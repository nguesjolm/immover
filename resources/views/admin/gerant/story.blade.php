...
<div class="card mb-3">
    ...
    <div class="card-header">
     <h5 class="mb-0">Offreurs actifs  => <b class="text-success">{{count(ReadgerantStat(0))}} Offreurs</b></h5>
    </div>
  ...
  <tbody class="bg-white">
    @foreach(ReadgerantStat(0) as $gerant)
    <tr>
      <td>{{$gerant->created_at}}<br> {{$gerant->nom}} - {{$gerant->phone}} <br><span class="badge badge-pill badge-warning">{{$gerant->identity}}</span></td>
      <td>
      ...
      <td>
        <a href="lockGerant?idgerant={{$gerant->idgerant}}">
