...
<div class="card mb-3">
    ...
    <div class="card-header">
     <h5 class="mb-0">Demandes Publiées  => <b class="text-success">{{count(ReadDemand(3))}} Demandes</b></h5>
    </div>
     ...
     <table class="table table-sm table-dashboard data-table no-wrap mb-0 fs--1 w-100">
      ...
      @foreach(ReadDemand(3) as $demand)
       <tr>
          ...
          <td>{{ReadOperaID($demand->opera)->operation}}</td>
          <td>{{ReadTypesID($demand->typesbiens)->types}}</td>
          ...
          <td>
              <a href="DelDemnd?Demnd={{$demand->demandeID}}&&url=newDemand">
                <span class="badge badge-pill badge-danger">
                Annuler
                </span>
              </a>

              <a href="TrouvDemnd?Demnd={{$demand->demandeID}}&&url=newDemand">
