...
<div class="card bg-light mb-3">
  <div class="card-body p-3">
      ...
      </a>, Veuillez consulter l'Historique des Abonnés ImmOver</strong>
      </p>
  </div>
</div>
...
<div class="card mb-3">
    ...
    <div class="card-header">
     <h5 class="mb-0">Abonnés  => <b class="text-success">{{count(ReadClientAll())}} Abonnés</b></h5>
    </div>
     ...
     <table class="table table-sm table-dashboard data-table no-wrap mb-0 fs--1 w-100">
      <thead class="bg-200">
        <tr>
          ...
          <th class="sort">Password</th>
        </tr>
      </thead>
      <tbody class="bg-white">
       @foreach(ReadClientAll() as $client)
        <tr>
           <td>{{$client->nom}}</td>
           <td>{{$client->phone}}</td>
           <td>{{$client->mail}}</td>
