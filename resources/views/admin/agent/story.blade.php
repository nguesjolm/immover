  ...
  <div class="card-body p-3">
      <p class="fs--1 mb-0"><a href="#!">
        ...
        <strong class="text-warning">
         Gestion des Agents LogRapid
        </strong>
      </a>, Veuillez consulter l'Historique des Agents</strong>
      </p>
  </div>
</div>
...
<div class="card mb-3">
    ...
    <div class="card-header">
     <h5 class="mb-0">Agents  => <b class="text-success">{{count(ReadAgent())}} Agents</b></h5>
    </div>
  ...
  <tbody class="bg-white">
    ...
    <tr>
      <td>{{$agent->nomagent}} <br> {{$agent->phone}}<br>{{$agent->mail}}</td>
      <td><b>{{$agent->statut}}</b></td>
      <td>{{$agent->codeagent}}</td>
      <td>
        <a href="agentdel?id={{$agent->idagents}}">
